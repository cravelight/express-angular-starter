...
gulp.task('vet', ['plato'], function() {
  log('Analyzing source with eslint and plato');
...
gulp.task('templatecache', ['clean-code'], function() {
  ...
  return gulp
    ...
    .pipe($.htmlmin({caseSensitive: true, collapseWhitespace: true}))
...
gulp.task('build', ['optimize', 'images', 'fonts'], function() {
  ...
  var msg = {
    ...
    message: 'Run `gulp serve-build` to see it in action'
  };
  del(config.temp);
  log(msg);
  notify(msg);
...
gulp.task('clean', function(done) {
  ...
  del(delconfig).then(function (paths) {
    done(); // ok
  }, function (reason) {
    done('Failed to delete files ' + reason); // fail
  });
});
